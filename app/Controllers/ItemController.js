...
class ItemController {
  static index(req, res) {
    ...
    const [rows, total] = ItemService.listItems(safeSkip, safeLimit, filters);
    res.json({
  ...
  static show(req, res) {
    ...
    const row = ItemService.getById(itemId);
  ...
  static update(req, res) {
    const itemId = parseInt(req.params.item_id, 10);
    const body = req.body || {};
    const row = ItemService.update(itemId, body);
    res.json(ApiSerializer.item(row.item, row.category));
  }
